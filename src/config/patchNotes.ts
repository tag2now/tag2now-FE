export type PatchNote = {
  version: string
  items: string[]
}

/**
 * Newest first. The top entry's version doubles as the "seen" marker in
 * localStorage, so adding an entry here is what re-shows the dialog — there is
 * no second version constant to keep in sync.
 */
export const PATCH_NOTES: PatchNote[] = [
  {
    version: '2.6',
    items: [
      '화면 전체 디자인 개편 — 색·글꼴·간격을 한 기준으로 통일',
      '"개요" 탭이 "홈"으로 바뀌고, 탭이 지금 벌어지는 일 순서로 재배치',
      '홈 지표 카드를 지금 접속 / 오늘 접속자 / 모집 중인 예약으로 교체',
      '리더보드·주간 철악귀·홈 요약이 같은 형식으로 표시되고, 순위 기준이 별도 열로 노출',
      '매칭 두 탭 모두 방 하나가 한 줄로 표시되고, 남은 자리 확인 가능',
      '게시글에 분류와 캐릭터를 따로 태그 가능 (태그팀 2명까지)',
      '통계를 한 화면에서 모두 확인, 자주 함께한 플레이어를 눌러 바로 이동',
      '계급 43종 전체 지원 (Yaksa 위 7계급 포함)',
      '모바일에서 이름·전적이 잘리던 문제 전반 수정',
    ],
  },
  {
    version: '2.5',
    items: [
      '게시글에 YouTube 링크를 첨부해 미리보기와 영상 재생 가능 (Shorts 지원)',
      '내가 작성한 게시글의 제목·내용·유형 수정과 첨부 영상 교체·제거 가능',
      '글쓰기·수정 화면의 좌우 여백을 줄여 입력 영역을 넓게 표시',
      '예약 상세에서 참여자 명단·계급과 남은 모집 자리 확인 가능',
      '예약 댓글을 Enter 키로 등록 가능',
    ],
  },
  {
    version: '2.4',
    items: [
      '예약 상세에서 댓글을 읽고 쓸 수 있음',
      '개요에서 모집 중인 예약과 최신 게시글이 그래프보다 위로 이동',
      '개요의 지표 카드와 순위 행을 눌러 해당 화면으로 바로 이동',
      '개요 예약 카드에 방장의 보유 계급 표시',
      '개요 화면에 움직임 추가 (수치 카운트, 상위 3위 강조)',
      '새로고침이 진행 중임을 버튼에서 확인 가능',
      '예약을 만들 때 플레이 예상 시간을 더 묻지 않음',
      '유저명에 한글·이모지는 아직 쓸 수 없음 (영문·숫자로 입력)',
    ],
  },
  {
    version: '2.3.3',
    items: [
      '예약 목록에서 보유 계급이 여러 개일 때 모두 표시 (상세에서 전체 확인)',
      '자정을 넘겨도 예약 목록이 끊기지 않고 새벽 6시까지 이어짐',
    ],
  },
  {
    version: '2.3.1',
    items: [
      '예약 매치 종류 필터를 토글로 변경 (전체 / 랭크매치 / 플레이어 매치)',
      '모바일 리더보드에서 서브 캐릭터가 보이지 않던 문제 수정',
      '모바일 매칭 목록에서 플레이어 이름이 잘리던 문제 수정',
    ],
  },
  {
    version: '2.3',
    items: [
      '개요의 최신 게시글 / 모집 중인 예약을 눌러 바로 해당 항목으로 이동',
      '게시글과 예약에 고유 주소가 생겨 링크 공유 가능',
      '브라우저 뒤로 가기로 이전 화면으로 돌아가기',
    ],
  },
  {
    version: '2.2',
    items: [
      '"개요" 탭 추가 — 접속자, 활성 방, 오늘 최대 접속, 등록 플레이어를 한눈에',
      '개요에서 리더보드 TOP 5 / 주간 철악귀 / 모집 중인 예약 / 최신 게시글 요약 확인',
      '접속 시 첫 화면이 개요로 변경',
    ],
  },
  {
    version: '2.1.1',
    items: ['예약 매치 종류에 "상관없음" 추가 (양쪽 필터에 모두 표시)'],
  },
  {
    version: '2.1',
    items: [
      '리더보드에 100위 밖 플레이어도 표시',
      '리더보드 이름 검색 및 캐릭터 필터 추가',
      '내가 만든 예약 수정 / 삭제 가능',
    ],
  },
  {
    version: '2.0.1',
    items: ['Yaksa 까지 계급 이미지 추가'],
  },
  {
    version: '2.0',
    items: [
      '신규 서버로 변경 (rpcn.tag2now.click)',
    ],
  },
  {
    version: '1.1',
    items: [
      '유저 이름 클릭 시 플레이어 히스토리 패널 오픈\n(리더보드, 랭크매치, 주간 철악귀)',
      '플레이어 히스토리에 활동 시간대 차트 추가',
    ],
  },
]

export const LATEST_PATCH_VERSION = PATCH_NOTES[0].version

/** How many lines the collapsed dialog is allowed to render — a version heading
 * counts as one, and so does each of its items.
 */
export const PATCH_NOTE_LINE_BUDGET = 10

/** The newest releases that fit the budget, whole versions only.
 *
 * A version is taken all-or-nothing: half a release reads as if the rest of it
 * never shipped. The first is always included, so a single release longer than
 * the budget still shows in full rather than collapsing to nothing.
 */
export function recentPatchNotes(notes: PatchNote[] = PATCH_NOTES): PatchNote[] {
  const taken: PatchNote[] = []
  let lines = 0
  for (const note of notes) {
    const cost = 1 + note.items.length
    if (taken.length > 0 && lines + cost > PATCH_NOTE_LINE_BUDGET) break
    taken.push(note)
    lines += cost
  }
  return taken
}
